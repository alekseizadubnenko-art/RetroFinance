            ...
            case '50/30/20':
                return (
                    <Card className="bg-retro-bg flex flex-col gap-5 h-full">
                        ...
                        {[
                            { label: 'Нужды (50%)', val: 50, amount: '₽' + (stats.income * 0.5).toLocaleString(), icon: 'check_circle', iconColor: 'text-accent-orange' },
                            { label: 'Желания (30%)', val: 30, amount: '₽' + (stats.income * 0.3).toLocaleString(), icon: 'check_circle', iconColor: 'text-accent-orange' },
                            { label: 'Сбережения (20%)', val: 20, amount: '₽' + (stats.income * 0.2).toLocaleString(), icon: 'error', iconColor: 'text-accent-orange', error: 'Требуется пополнение' }
                        ].map((item, i) => (
                            <div key={i} className="flex flex-col gap-2">
                                <div className="flex justify-between items-center text-sm font-bold uppercase">
                                    ...
                                    <div className="flex gap-2">
                                        <span className={item.error ? 'text-red-500' : 'text-accent-orange'}>{item.val}%</span>
                                        <span className="font-mono">{item.amount}</span>
                                    </div>
                                </div>
                                <ProgressBar value={item.val} max={100} />
                                {item.error && <p className="text-[10px] text-red-500 uppercase font-bold mt-1">{item.error}</p>}
                            </div>
                        ))}
                    </Card>
                );
            case '70/20/10':
                return (
                    <Card className="bg-retro-bg flex flex-col gap-5 h-full">
                         ...
                         {[
                            { label: 'Жизнь (70%)', val: 70, amount: '₽' + (stats.income * 0.7).toLocaleString(), icon: 'home', iconColor: 'text-gray-700' },
                            { label: 'Сбережения (20%)', val: 20, amount: '₽' + (stats.income * 0.2).toLocaleString(), icon: 'savings', iconColor: 'text-accent-orange' },
                            { label: 'Инвестиции (10%)', val: 10, amount: '₽' + (stats.income * 0.1).toLocaleString(), icon: 'trending_up', iconColor: 'text-accent-green' }
                        ].map((item, i) => (
                            <div key={i} className="flex flex-col gap-2">
                                <div className="flex justify-between items-center text-sm font-bold uppercase">
                                    <div className="flex items-center gap-2">
                                    ...
                                    <div className="flex gap-2">
                                        <span className="text-accent-orange">{item.val}%</span>
                                        <span className="font-mono">{item.amount}</span>
                                    </div>
                                </div>
                                <ProgressBar value={item.val} max={100} />
                    ...
                    <Card className="bg-retro-bg flex flex-col gap-5 h-full">
                        ...
                        <div className="flex flex-col gap-4">
                            ...
                            <div className="text-sm text-gray-600 leading-relaxed">
                                Оставшиеся <span className="font-bold text-retro-border">90% (₽{(stats.income * 0.9).toLocaleString()})</span> распределите на долги и текущие расходы.
                            </div>
                        </div>
                    </Card>
                );
                        ...
                        <div className="flex flex-col gap-2 mt-2">
                            ...
                            <div className="flex justify-between items-center border-b-2 border-retro-border pb-2">
                             ...
                             <div className={`flex justify-between items-center pt-2 ${isBalanced ? 'text-accent-green' : 'text-retro-action'}`}>
                                <span className="font-bold uppercase text-sm">{isBalanced ? 'Бюджет сошелся!' : 'Не распределено'}</span>
            ...
            <Card className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-[#fdf6e3]">
                <div className="flex flex-col md:flex-row md:items-center gap-2 md:gap-6 text-left w-full">
                    <div className="font-bold text-sm md:text-base uppercase flex flex-col md:block">
                        Финансовое здоровье: <span className="text-[#d97706] mt-1 md:mt-0">Удовлетворительно</span>
                    </div>
                    <div className="hidden md:block w-1.5 h-1.5 bg-retro-border rounded-full"></div>
                    <div className="text-xs md:text-sm uppercase tracking-tight flex flex-col md:block text-gray-600 md:text-retro-border">
                        <span>Свободных средств: ₽{stats.totalBalance.toLocaleString()}</span>
                        <span className="md:hidden">Норма сбережений: 12.5%</span>
                    </div>
                </div>
                <div className="text-left md:text-right w-full md:w-auto shrink-0 flex flex-row md:flex-col justify-between md:justify-start items-center md:items-end border-t-2 md:border-t-0 border-retro-border/10 pt-2 md:pt-0 mt-2 md:mt-0">
                    ...
                    <div className="text-[10px] font-bold uppercase tracking-widest opacity-60">Свободные средства</div>
                </div>
            </Card>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
                <div className="lg:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6">
                    <Card color="beige" className="flex flex-col justify-between min-h-[140px] md:min-h-[160px]">
                        ...
                        <div>
                            <div className="text-2xl md:text-3xl font-bold mt-2">₽{stats.totalBalance.toLocaleString()}</div>
                            <div className="text-xs font-medium mt-1 uppercase opacity-70">На всех счетах</div>
                        </div>
                    </Card>
                    <Card color="beige" className="flex flex-col justify-between min-h-[140px] md:min-h-[160px]">
                        <div className="flex justify-between items-start">
                    ...
                    <Card color="pink" className="flex flex-col justify-between min-h-[140px] md:min-h-[160px]">
                        ...
                        <div>
                            ...
                            <div className="text-xs font-medium mt-1 uppercase opacity-70">Всего транзакций</div>
                        </div>
                    </Card>
                    <Card color="pink" className="flex flex-col justify-between min-h-[140px] md:min-h-[160px]">
                        <div className="flex justify-between items-start">
                            <span className="text-xs font-bold uppercase tracking-wider">Баланс долгов</span>
                            <div className="border-2 border-retro-border p-1 bg-accent-orange text-white">
                ...
                {/* Accounts List */}
                <div className="flex flex-col gap-4">
                    <h3 className="font-bold uppercase text-sm mb-1">Счета</h3>
                    {['Наличные', 'Альфа', 'Альфа-бизнес', 'Тиньков'].map((acc, i) => (
                        <div key={i} className="bg-white border-2 border-retro-border p-3 flex justify-between items-center shadow-retro-sm hover:translate-x-[2px] transition-transform cursor-pointer">
                            <span className="font-medium text-sm">{acc}</span>
                            <span className="font-bold font-mono">₽0</span>
                        </div>
                    ))}
                    <div className="text-center text-xs font-bold uppercase cursor-pointer hover:underline mt-2">
                        +2 еще
                    </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-8">
                ...
                <Card className="flex flex-col justify-between gap-6 h-full">
                    <div>
                        ...
                        <p className="text-xs uppercase font-medium text-gray-600">Подушка безопасности на 4.0 месяцев расходов</p>
                    </div>
                    <div className="border-2 border-retro-border bg-red-50 p-3 flex items-start gap-3">
                        ...
                        <p className="text-xs text-red-600 font-bold uppercase leading-relaxed">Критически низкий уровень. Рекомендуется срочно пополнить фонд.</p>
                    </div>
                    <div className="flex flex-col gap-2">
                        ...
                        <ProgressBar value={5} max={100} height="h-4" />
                    </div>
                    <div className="flex justify-between items-end border-t-2 border-gray-100 pt-4">
                        <div>
                            ...
                            <div className="text-xl font-bold font-mono">₽9 090,17</div>
                        </div>
                        <div className="text-right">
                            <div className="text-[10px] uppercase font-bold text-gray-500">Осталось до цели</div>
                            <div className="text-xl font-bold font-mono text-accent-orange">₽190 909,83</div>
                        </div>
                    </div>
                </Card>
            </div>
                ...
                <Card className="lg:col-span-2 p-0 flex flex-col gap-0 overflow-hidden">
                    <div className="p-4 border-b-2 border-retro-border flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 bg-retro-bg">
                        ...
                        <div className="flex gap-2 w-full sm:w-auto">
                             <button className="flex-1 sm:flex-none px-3 py-1 bg-white border-2 border-retro-border text-xs font-bold uppercase hover:bg-red-50 text-red-500 shadow-retro-sm active:translate-y-[2px] active:shadow-none transition-all">- Расход</button>
                             <button className="flex-1 sm:flex-none px-3 py-1 bg-accent-orange text-white border-2 border-retro-border text-xs font-bold uppercase hover:bg-orange-600 shadow-retro-sm active:translate-y-[2px] active:shadow-none transition-all">+ Доход</button>
                        </div>
                    </div>
                    <div className="divide-y-2 divide-retro-border">
                        {recentTransactions.length > 0 ? recentTransactions.map((t, i) => (
                            <div key={i} className="p-4 flex items-center gap-4 hover:bg-gray-50 transition-colors group cursor-pointer">
                                <div className="size-10 border-2 border-retro-border flex items-center justify-center bg-card-pink shrink-0 shadow-retro-sm group-hover:shadow-none group-hover:translate-x-[2px] group-hover:translate-y-[2px] transition-all">
                                    <span className="material-symbols-outlined -rotate-45 text-sm">{t.type === 'income' ? 'arrow_upward' : 'arrow_downward'}</span>
                                </div>
                                <div className="flex-1 min-w-0">
                                    <h4 className="font-bold text-sm truncate">{t.title}</h4>
                                    <p className="text-xs text-gray-600 truncate">{t.category} • {t.source}</p>
                                </div>
